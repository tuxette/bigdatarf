...
"myrandomForest.formula" <-
  function(formula, data = NULL, ..., subset, na.action = na.fail) {
    ### formula interface for myrandomForest.
    ...
    ## test with:
    ## myrandomForest(Fertility~.-Catholic+I(Catholic<50),data=swiss,mtry=2)
    ...
    ret <- myrandomForest.default(m, y, ...)
    cl <- match.call()
...
"myrandomForest.default" <-
  function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500,
           ...
           # print("myRF function !")
             ...
             # if ((!Stratify) && sampsize > nrow(x)) print("sampsize too large")
